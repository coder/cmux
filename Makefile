...
screenshot-storybook: node_modules/.installed ## Generate screenshots of Storybook stories (use STORY= or COMPONENT= to filter)
	@if [ -n "$(STORY)" ]; then \
		bun scripts/screenshot-storybook.ts --story $(STORY); \
	elif [ -n "$(COMPONENT)" ]; then \
		bun scripts/screenshot-storybook.ts --component $(COMPONENT); \
	else \
