...
describeIntegration("Workspace script execution", () => {
  ...
  describe.each<{ type: "local" | "ssh" }>([{ type: "local" }, { type: "ssh" }])(
    ...
    ({ type }) => {
      ...
      test.concurrent(
        "writes CMUX_OUTPUT and CMUX_PROMPT when executing workspace script",
        async () => {
...
if [ -n "\${CMUX_OUTPUT:-}" ]; then
  printf "Toast via CMUX_OUTPUT" > "\${CMUX_OUTPUT}"
fi

if [ -n "\${CMUX_PROMPT:-}" ]; then
  printf "Prompt via CMUX_PROMPT" > "\${CMUX_PROMPT}"
fi
EOF
...
`;
              ...
              expect(executionResult.data.outputFile).toBe("Toast via CMUX_OUTPUT");
              expect(executionResult.data.promptFile).toBe("Prompt via CMUX_PROMPT");
            } finally {
              await cleanup();
            }
          } finally {
