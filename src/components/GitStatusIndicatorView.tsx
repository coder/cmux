...
export const GitStatusIndicatorView: React.FC<GitStatusIndicatorViewProps> = ({
  ...
  const renderIndicators = (indicators: string) => {
    return (
      <span className="mr-2 shrink-0 font-mono whitespace-pre text-gray-500">
  ...
  const renderBranchHeaders = () => {
    ...
    return (
      <div className="border-gray-900-light mb-2 flex flex-col gap-0.5 border-b pb-2">
        {branchHeaders.map((header, index) => (
          <div key={index} className="flex gap-2 font-mono leading-snug">
            ...
            <span className="text-gray-200">[{header.branch}]</span>
          </div>
        ))}
      </div>
    );
  ...
  const renderDirtySection = () => {
    ...
    return (
      <div className="border-gray-900-light mb-2 border-b pb-2">
        <div className="text-git-dirty mb-1 font-mono font-semibold">Uncommitted changes:</div>
        <div className="flex flex-col gap-px">
          {displayFiles.map((line, index) => (
            <div
              key={index}
              className="font-mono text-[11px] leading-snug whitespace-pre text-gray-200"
            >
              {line}
            </div>
          ))}
        ...
        {isTruncated && (
          <div className="text-gray-500-light mt-1 text-[10px] italic">
          ...
          {commits.map((commit, index) => (
            <div key={`${commit.hash}-${index}`} className="flex flex-col gap-0.5">
              <div className="flex gap-2 font-mono leading-snug">
                ...
                <span className="text-gray-500-light shrink-0">{commit.date}</span>
                <span className="flex-1 break-words text-gray-200">{commit.subject}</span>
              </div>
            </div>
          ))}
        </div>
  ...
  const tooltipElement = (
    <div
      className={cn(
        "fixed z-[10000] bg-gray-900 text-gray-200 border border-gray-900-light rounded px-3 py-2 text-[11px] font-mono whitespace-pre max-w-96 max-h-[400px] overflow-auto shadow-[0_4px_12px_rgba(0,0,0,0.5)] pointer-events-auto transition-opacity duration-200",
