...
export const UserMessage: React.FC<UserMessageProps> = ({
  ...
  return (
    ...
    >
      {content && (
        <pre className="m-0 font-mono text-xs leading-4 break-words whitespace-pre-wrap text-neutral-400 opacity-90">
          {content}
        </pre>
      )}
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {message.imageParts.map((img, idx) => (
            <img
              ...
              className="max-h-[300px] max-w-80 rounded border border-neutral-800"
            />
          ))}
        </div>
      )}
