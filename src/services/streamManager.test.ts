...
describe("StreamManager - Unavailable Tool Handling", () => {
  ...
  // Integration test - requires API key and TEST_INTEGRATION=1
  // This test verifies that when a model requests an unavailable tool,
  // we get an immediate error instead of hanging indefinitely
  describeIntegration("with real API", () => {
    test("should handle model requesting unavailable tool", async () => {
      const workspaceId = "test-workspace-unavailable-tool-real";
      const anthropic = createAnthropic({ apiKey: process.env.ANTHROPIC_API_KEY });
      const model = anthropic("claude-sonnet-4-5");
      ...
      // Suppress error events to avoid uncaught exceptions
      streamManager.on("error", () => undefined);
        ...
        [
          {
            role: "user",
            content:
              "Use the file_edit_replace tool to edit a file. You must specifically use file_edit_replace.",
          },
        ],
        model,
        "anthropic:claude-sonnet-4-5",
        1,
        "You are a helpful assistant. When asked to use a specific tool, you must attempt to use exactly that tool.",
        undefined,
        {} // No tools available - file_edit_replace doesn't exist
      );
      ...
      // Look for file_edit_replace tool calls
      const fileEditStart = toolEvents.find(
        (e) => e.type === "tool-call-start" && e.toolName === "file_edit_replace"
      );
      const fileEditEnd = toolEvents.find(
        (e) => e.type === "tool-call-end" && e.toolName === "file_edit_replace"
      );

      if (fileEditStart && fileEditEnd) {
        // Model requested the unavailable tool - verify we handled it properly
        expect(fileEditStart).toBeDefined();
        expect(fileEditEnd).toBeDefined();

        // Verify error result
        const errorResult = fileEditEnd.result as { error?: string };
        expect(errorResult?.error).toContain("not available");

        // Verify it resolved quickly (under 2 seconds from start to end)
        const duration = fileEditEnd.timestamp - fileEditStart.timestamp;
        expect(duration).toBeLessThan(2000); // Should error immediately, not hang
      } else {
        // Model might have refused to use the tool or explained it couldn't
        // This is also acceptable behavior - just log it
        console.log(
          "Model did not attempt to use unavailable tool (possibly refused or explained limitation)"
        );
      }
    });
  });
