...
export const createBashTool: ToolFactory = (config: ToolConfiguration) => {
  return tool({
    ...
    execute: async (
      ...
      const finalScript =
        config.niceness !== undefined ? `nice -n ${config.niceness} bash -c ${JSON.stringify(script)}` : script;
      const finalCommand = config.niceness !== undefined ? ["bash", "-c", finalScript] : ["bash", "-c", script];
