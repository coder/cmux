...
exports.StreamingMessageAggregator = void 0;
const message_1 = require("../../types/message");
const toolParts_1 = require("../../types/toolParts");
...
class StreamingMessageAggregator {
    ...
    streamSequenceCounter = 0; // For ordering parts within a streaming message
    ...
    getAllMessages() {
        ...
        this.cachedMessages = Array.from(this.messages.values()).sort((a, b) => (a.metadata?.historySequence ?? 0) - (b.metadata?.historySequence ?? 0));
        return this.cachedMessages;
    }
    ...
    handleStreamStart(data) {
        ...
        const isCompacting = lastUserMsg?.metadata?.toolPolicy?.some((filter) => filter.action === "require" &&
            new RegExp(`^${filter.regex_match}$`).test("compact_summary")) ?? false;
        const context = {
            startTime: Date.now(),
            isComplete: false,
            isCompacting,
        ...
        const streamingMessage = (0, message_1.createCmuxMessage)(data.messageId, "assistant", "", {
                    ...
                    for (const backendPart of data.parts) {
                        if (backendPart.type === "dynamic-tool") {
                            ...
                            const toolPart = message.parts.find((part) => part.type === "dynamic-tool" &&
                                part.toolCallId === backendPart.toolCallId);
                            if (toolPart) {
            ...
            if (message.role === "user") {
                ...
                for (let i = mergedParts.length - 1; i >= 0; i--) {
                    ...
                    if (part.type === "reasoning" ||
                        ...
                        (0, toolParts_1.isDynamicToolPart)(part)) {
                        lastPartIndex = i;
                        break;
                    }
                }
                mergedParts.forEach((part, partIndex) => {
                    ...
                    else if ((0, toolParts_1.isDynamicToolPart)(part)) {
...
exports.StreamingMessageAggregator = StreamingMessageAggregator;
