...
exports.stripReasoningForOpenAI = stripReasoningForOpenAI;
...
function filterEmptyAssistantMessages(messages) {
    return messages.filter((msg) => {
        ...
        const hasContent = msg.parts.some((part) => (part.type === "text" && part.text) || part.type === "dynamic-tool");
        return hasContent;
    });
}
/**
 ...
 */
function stripReasoningForOpenAI(messages) {
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        if (textParts.length === 0) {
            if (hasToolResults) {
                ...
                const resultIds = new Set(toolMsg.content
                    .filter((r) => r.type === "tool-result")
                    .map((r) => (r.type === "tool-result" ? r.toolCallId : "")));
                const validToolCalls = toolCallParts.filter((p) => p.type === "tool-call" && resultIds.has(p.toolCallId));
                ...
                for (const group of groups) {
                    if (group.parts.length > 0) {
                        ...
                        if (group.type === "tool-call" && hasToolResults) {
                            ...
                            partsToInclude = group.parts.filter((p) => p.type === "tool-call" && resultIds.has(p.toolCallId));
                        }
                        ...
                        if (partsToInclude.length > 0) {
                            ...
                            if (group.type === "tool-call" && hasToolResults) {
                                ...
                                const toolCallIds = new Set(partsToInclude
                                    .filter((p) => p.type === "tool-call")
                                    .map((p) => (p.type === "tool-call" ? p.toolCallId : ""))
                                    .filter(Boolean));
                                // Filter the tool results to only include those for these tool calls
                                const relevantResults = toolMsg.content.filter((r) => r.type === "tool-result" && toolCallIds.has(r.toolCallId));
 ...
 */
function transformModelMessages(messages, provider) {
